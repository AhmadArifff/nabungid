...
export default function AdminDashboardPage() {
  const { pendingLedgers, pendingWithdrawals, items, bundles } = useAdminStore();

  const totalKasEstimasi = 45000000; // Mock total kas
  const totalNasabah = 28;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-5 rounded-3xl bg-slate-900/80 border border-white/10 flex flex-col justify-between">
          ...
          <div>
            ...
            <div className="text-[10px] text-slate-400 mt-1 flex items-center space-x-1">
              ...
              <span>Akumulasi dari 28 nasabah</span>
            </div>
          </div>
        </div>
        ...
        <div className="p-5 rounded-3xl bg-slate-900/80 border border-white/10 flex flex-col justify-between">
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-semibold text-slate-400">Total Nasabah Aktif</span>
            <div className="w-8 h-8 rounded-xl bg-blue-500/20 text-blue-400 flex items-center justify-center">
              <Users className="w-4 h-4" />
            </div>
          </div>
          <div>
            <div className="text-2xl font-black font-mono text-white">{totalNasabah} Orang</div>
            <div className="text-[10px] text-slate-400 mt-1">100% disiplin menabung</div>
          </div>
        </div>
