...
import { Coins, User, ShieldCheck, ArrowRight, Lock, Phone, Sparkles, CheckCircle2 } from 'lucide-react';
  ...
  return (
    <div className="w-full max-w-md mx-auto">
      ...
      <div className="p-6 sm:p-8 rounded-3xl bg-slate-900/80 border border-white/10 backdrop-blur-xl shadow-2xl relative overflow-hidden">
        ...
        <div className="grid grid-cols-2 gap-1.5 p-1 rounded-2xl bg-slate-950/80 border border-white/10 mb-6">
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-medium text-slate-300 mb-1.5">
              {selectedRole === 'NASABAH' ? 'Nomor WhatsApp / Email' : 'Email Administrator'}
            </label>
            <div className="relative">
              ...
              <input
                ...
                placeholder={selectedRole === 'NASABAH' ? '081234567890' : 'admin@nabungid.com'}
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full py-3.5 px-4 rounded-xl bg-gradient-to-r from-amber-500 via-amber-400 to-amber-500 text-slate-950 font-bold text-sm shadow-xl shadow-amber-500/20 hover:brightness-110 active:scale-[0.99] transition-all flex items-center justify-center space-x-2 mt-6"
          >
            {isLoading ? (
